-- Block cleaner-link reissue while a proof task is submitted (additive).
...
-- Preserves submitted task rows and photos. Needs-attention reissue remains allowed.
...
begin
  ...
  -- Real token rotation only while open or needs_attention. Submitted keeps evidence intact.
  ...
  where public.housekeeping_proof_tasks.status in ('open', 'needs_attention')
